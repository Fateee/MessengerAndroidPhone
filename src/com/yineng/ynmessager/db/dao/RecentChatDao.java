...
public class RecentChatDao {
	...
	public void updateUnreadCount(String userNo, int chatType, int unReadCount) {
		...
		mDB.update(mTable, contentValues, "userNo = ? AND chatType = ?",
				new String[] { userNo, chatType + "" });
	}
	...
	public List<RecentChat> queryRecentChatPage(int pageindex, int count) {
		...
		String sql = "select * from RecentChat ORDER BY isTop desc,topTime desc ,dateTime desc limit "
		...
		if (cursor != null && cursor.getCount() > 0) {
			while (cursor.moveToNext()) {
				...
				chat.setChatType(cursor.getInt(cursor
						.getColumnIndex("chatType")));
				chat.setSenderName(cursor.getString(cursor
						.getColumnIndex("senderName")));
				chat.setSenderName(cursor.getString(cursor
						.getColumnIndex("senderNo")));
				chat.setDateTime(cursor.getString(cursor
						.getColumnIndex("dateTime")));
				chat.setHeadUrl(cursor.getString(cursor
						.getColumnIndex("headUrl")));
				chat.setHeadLocalPath(cursor.getString(cursor
						.getColumnIndex("headLocalPath")));
				chat.setIsTop(cursor.getInt(cursor.getColumnIndex("isTop")));
				chat.setUnReadCount(cursor.getInt(cursor
	...
	public RecentChat isChatExist(String userNo, int type) {
		...
		RecentChat chat = null;
		Cursor cursor = mDB.query(mTable, null, "userNo = ? AND chatType = ?",
				new String[] { userNo, type + "" }, null, null, null);
		if (cursor != null && cursor.getCount() > 0) {
...
//		mDB.execSQL(sql);
	...
	public String getUserNameByUserId(String userId, int type) {
		...
		case Const.CHAT_TYPE_DIS:
			name = "讨论组:" + userId;
			sql = "select * from DiscussGroup where groupName ='"
					+ userId.trim() + "'";
			break;
		case Const.CHAT_TYPE_GROUP:
			name = "群组:" + userId;
			sql = "select * from UserGroup where groupName ='"
					+ userId.trim() + "'";
			break;
		}
		Cursor cursor = mDB.rawQuery(sql, null);
