...
public class GroupChatActivity extends BaseActivity implements OnClickListener, ReceiveMessageCallBack, ReceiveReqIQCallBack{
	...
	public boolean onContextItemSelected(MenuItem item)
	{
		...
		switch(itemId)
		{
			case 0:  //复制文本
				...
				break;
			case 1:  //删除
				int deletedRows = mGroupChatDao.deleteByPacketId(viewItem.getPacketId());
				L.i(TAG,"删除了" + deletedRows + "条会话消息");
				refreshUIByPageIndex(viewItem);
				// 得到最后一条消息的内容，设置到最近会话消息列表中更新显示
				GroupChatMsgEntity lastest = mGroupChatDao.queryLastestChatMsg(mChatUserNum);
				String content;
				if(lastest != null)
				{
	...
	private void initData() {
		...
		L.e("mChatUserNum == "+mChatUserNum);
		String myUserNo = LastLoginUserSP.getInstance(GroupChatActivity.this)
				.getUserAccount();
...
//		mCommonHeadViewRL = (RelativeLayout) findViewById(R.id.group_chat_title_layout);
//		mGroupTitleTV = (TextView) mCommonHeadViewRL.findViewById(R.id.chat_common_title_view_name);
...
//		switch (v.getId()) {
//		case R.id.iv_common_group_chat_back:
//			finish();
//			break;
//		case R.id.tv_common_group_chat_info_btn:
...
//			msg.setTo(JIDUtil.getSendToMsgAccount(mChatUserNum));
			...
			try {
				...
				mGroupChatDao.saveOrUpdate(mGroupChatMsgEntity);
			}
			break;
